...
"""InteractiveMarker static/dynamic obstacles → PointCloud2 for controller sim."""
...
class Obstacle:
    ...
    height: float
    dynamic: bool = False
    # Dynamic motion: circle orbit around (cx, cy)
    orbit_cx: float = 0.0
    orbit_cy: float = 0.0
    orbit_r: float = 0.0
...
class ObstacleWorld:
    ...
    def sample_cloud(self, resolution: float = 0.05) -> np.ndarray:
        ...
        for obs in self.obstacles:
            ...
            # Fill disk on floor for marking.
...
class ObstacleMarkerNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('cloud_topic', 'controller_sim/obstacle_cloud')
        self.declare_parameter('publish_rate_hz', 10.0)
        self.declare_parameter('sample_resolution', 0.08)
        self.declare_parameter('static_count', 3)
        self.declare_parameter('dynamic_count', 2)
        ...
        self._server = InteractiveMarkerServer(self, 'controller_obstacles')
    ...
    def _seed_obstacles(self, n_static: int, n_dynamic: int) -> None:
        # Keep static cylinders well outside the default circle path (r≈2) and
        # outside its inflated corridor so MPPI can keep lapping continuously.
        static_xy = [(3.4, 0.8), (-3.3, 1.9), (1.4, 3.5), (-3.0, -2.4), (3.2, -2.0)]
        ...
        for i in range(max(0, n_dynamic)):
            ...
            # Orbit outside the reference circle so graceful/RPP can follow r=2.
            self._world.obstacles.append(
                Obstacle(
                    name=f'dynamic_{i}',
                    x=2.8 * math.cos(phase),
                    y=2.8 * math.sin(phase),
                    radius=0.28,
                    height=1.0,
                    dynamic=True,
                    orbit_cx=0.0,
                    orbit_cy=0.0,
                    orbit_r=2.8 + 0.15 * i,
                    orbit_omega=0.35 * (1.0 if i % 2 == 0 else -1.0),
                    orbit_phase=phase,
    ...
    def _insert_marker(self, obs: Obstacle) -> None:
        ...
        im.scale = max(0.8, 2.5 * obs.radius)
        im.pose = _pose(obs.x, obs.y)

        # Single MOVE_PLANE control on the cylinder body (XY drag via Interact).
        # Orientation must be a unit quaternion; (w=y=1/√2) → XY plane.
        body = InteractiveMarkerControl()
        body.name = 'move_xy'
        body.always_visible = True
        body.interaction_mode = InteractiveMarkerControl.MOVE_PLANE
        body.orientation = Quaternion(w=0.7071068, x=0.0, y=0.7071068, z=0.0)
    ...
    def _on_feedback(self, feedback: InteractiveMarkerFeedback) -> None:
        ...
        if obs.dynamic:
            # Dragging a dynamic obstacle re-centers its orbit.
            obs.orbit_cx = obs.x
            obs.orbit_cy = obs.y
            obs.orbit_phase = 0.0
            self._t0 = self.get_clock().now()
            # Do not applyChanges during POSE_UPDATE — it fights RViz drag.
        ...
        # Static: accept pose only on mouse-up so the Interact drag is not reset.
        if feedback.event_type == InteractiveMarkerFeedback.MOUSE_UP:
            self._server.setPose(obs.name, _pose(obs.x, obs.y))
            self._server.applyChanges()
            self._publish_cloud()
        else:
            # Live cloud update while dragging (no applyChanges).
            self._publish_cloud()
