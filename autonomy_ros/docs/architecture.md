...
`autonomy_ros` 是 **ROS 2 集成层**：话题/服务桥接、外部指令与可视化；规划、控制、代价地图、行为树在 `autonomy` 核心，由 `autonomy_ros::RosAutonomySystem` 调用。

设计原则（重构后）：

- **单节点**：`autonomy_node` 承载全部 ROS 接口。
- **单导航入口**：`Autonomy::navigateToPose()`（BT 或直驱由 `use_bt_navigation` 决定，不再混用两套执行逻辑）。
- **单 odom 入口**：`SensorBridge` 订阅里程计，经 `Autonomy::dispatchOdom` 分发给核心与监听方。
- **单 costmap 写入**：`CostmapBridge` 持有 planner 共享 `Costmap2DWrapper`（地图 + 传感器入、costmap 话题出）。
- **薄 Bridge**：`TfBridge` / `MapBridge` / `CostmapBridge` / `SensorBridge` / `ActuationBridge`；`/plan` 由 `PlanningVisualization` 发布；Follow 检测仅在 `CommandInterface`。
...
├── Autonomy           # 核心门面：启动 system::AutonomyNode、导航、部分出站 I/O
│   ├── bridge::TfBridge
│   ├── bridge::MapBridge
│   ├── bridge::CostmapBridge
│   ├── bridge::SensorBridge
│   └── bridge::ActuationBridge
├── command::CommandInterface   # autonomy_msgs；内含 task::TaskManager（ROS 状态）
│   └── Autonomy 持有 autonomy::tasks::Task   # core TaskScheduler / BT
...
| 传感器入 | `bridge::SensorBridge` | `odom`、`/scan`、点云/range 等 → core / `CostmapBridge` |
| 代价地图 | `bridge::CostmapBridge` | 静态图 + 传感器 → wrapper；`global_costmap` / `local_costmap` 出 |
| 平台出 | `bridge::ActuationBridge` | `cmd_vel` 发布 |
| 导航 | `Autonomy` + 核心 `PlannerServer` / `ControllerServer` / `TaskScheduler` | 内部；路径经 `notifyPlan` → `PlanningVisualization` 发布 `/plan` |
...
| 任务状态 | `CommandInterface` → `task::TaskManager` | `autonomy/status`、`autonomy/events`、`autonomy/battery` |
| BT 调度 | `autonomy::tasks::Task` | 附着 `TaskScheduler`，由 `Autonomy::navigateToPose` 调用 |
...
sequenceDiagram
  ...
  AN->>A: start()
  A->>Core: CreateAutonomyNode(lua) + Start()
  ...
  opt enable_visualization
    AN->>VZ: start()
    AN->>A: dispatchOdom / notifyPlan → VZ
  end
  opt enable_command
    AN->>CI: start() + Follow 检测订阅
    A->>TM: dispatchOdom → updateOdom
  end
```
...
## 参数（`config/autonomy_params.yaml`）
...
flowchart LR
  ...
  DISPATCH --> TM[TaskManager.updateOdom]
  DISPATCH --> VZ[Visualizer.onRobotPose]
...
2. **BT 模式**：`TaskScheduler::NavigateToPose`；`controlStep` 经 `ActuationBridge` 发布 `GetLastCmdVel()`。
...
| 数据 | 来源 | Visualizer |
|------|------|------------|
| 路径 | `notifyPlan` | `Visualizer.onGlobalPath` |
| 机器人位姿 | `dispatchOdom` | `TaskManager` + `Visualizer.onRobotPose` |
...
| `autonomy_ros::RosAutonomySystem` | `autonomy` 核心 |
|--------------------------|-----------------|
...
- Follow `target_id`：`CommandInterface` 查检测表并变换到 `coreOptions().global_frame`。
...
| Visualizer 订阅 `/odom`、`/plan` | `dispatchOdom` / `notifyPlan` 直推 Visualizer |
