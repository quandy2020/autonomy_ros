...
std::unique_ptr<::autonomy::common::transform::Rigid3d> TfBridge::LookupToTracking(
    const ::autonomy::common::Time time,
    const std::string& frame_id) const 
{
    tf2::Duration timeout(tf2::durationFromSec(lookup_transform_timeout_sec_));
    std::unique_ptr<::autonomy::common::transform::Rigid3d> frame_id_to_tracking;
    try {
        // const rclcpp::Time latest_tf_time = buffer_->lookupTransform(tracking_frame_, frame_id, ::rclcpp::Time(0.), timeout).header.stamp;
        ...
        // }
        // return std::make_unique<::autonomy::transform::Rigid3d>(
        //     common::transform::ToRigid3d(buffer_->lookupTransform(tracking_frame_, frame_id, requested_time, timeout)));
