...
{
  visualizer_ = std::make_unique<viz::Visualizer>(node_);
  ...
  navigation_server_ = std::make_unique<navigation::NavigationServer>(
  ...
  if (options_.ros_options.publish_diagnostics) {
    diagnostics_ = std::make_unique<debug::DiagnosticsPublisher>(
      ...
      [this]() {
        ...
        if (navigation_server_) {
          snap.odometry_received = navigation_server_->hasOdometry();
          snap.navigation_active = navigation_server_->hasActiveNavigationTask();
          snap.controller_enabled = navigation_server_->isControllerEnabled();
        }
...
void RosAutonomySystem::startBridges()
{
  ...
  costmap_bridge_ = std::make_unique<bridge::CostmapBridge>(
    node_, core_->GetGlobalCostmap().get(), ros_opts);
  ...
  sensor_handlers.on_odom = [this](const nav_msgs::msg::Odometry::SharedPtr & msg) {
    ...
    if (navigation_server_) {
      navigation_server_->updateOdom(*msg);
    }
    if (visualizer_) {
