...
void CommandInterface::updateOdom(const nav_msgs::msg::Odometry & odom)
{
  if (task_manager_) {
...
CommandInterface::teleopCommand() const
{
  return task_manager_ ? task_manager_->teleopCommand() : std::nullopt;
}

bool CommandInterface::hasOdometry() const
{
  return task_manager_ && task_manager_->hasOdometry();
}

bool CommandInterface::hasActiveNavigationTask() const
{
  return task_manager_ && task_manager_->hasActiveTask();
}

bool CommandInterface::isControllerEnabled() const
{
...
, visualizer_(visualizer)
{
  ...
  startTeleopIngress(cmd_vel_teleop_topic);
    ...
    [this](const std::shared_ptr<autonomy_msgs::srv::SetTeleopMode::Request> & req,
      ...
      if (req->enable) {
            ...
            constants::msg::kEmpty, req->preempt_other_tasks))
        {
          ...
          task_manager_->beginTeleop(req->max_linear_speed, req->max_angular_speed);
          res->success = true;
          res->error = task_manager_->makeError(autonomy_msgs::msg::Error::NONE, constants::msg::kEmpty);
        }
      } else {
        task_manager_->cancelTask(constants::msg::kTaskTeleopSrv, false);
        task_manager_->endTeleop();
  ...
  geometry_msgs::msg::PoseStamped goal, const std::string & task_id)
{
  ...
  task_manager_->setControllerEnabled(true);
  const bool ok = task_manager_->navigateToPose(
  ...
  const std::function<bool()> & cancel_check)
{
  ...
  const double tol = task_manager_->coreOptions().goal_tolerance;
  ...
  task_manager_->replanToGoal(dock_pose);
  bool docked = false;
  spinUntilCancel(
    task_id,
  ...
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<NavigatePose>> handle)
{
  ...
  const bool ok = navigateToGoal(
    task_id, goal->goal, task_manager_->coreOptions().goal_tolerance, waypoint_timeout_sec_,
    ...
    spinUntilCancel(
      ...
      [&]() {
        ...
        if (goal->target.use_target_pose) {
          task_manager_->replanToGoal(goal->target.target_pose);
          ...
          if (!target_pose) {
            ...
            task_manager_->replanToGoal(*target_pose);
  ...
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<Teleop>> handle)
{
  ...
  const double time_allowance_sec =
    static_cast<double>(goal->time_allowance.sec) +
    static_cast<double>(goal->time_allowance.nanosec) * 1e-9;
  const auto start = std::chrono::steady_clock::now();

  const bool ok = task_manager_->teleopDrive(
    time_allowance_sec,
    static_cast<double>(goal->max_linear_speed),
    static_cast<double>(goal->max_angular_speed),
    [&]() { return handle->is_canceling(); });
  ...
  result->error = task_manager_->makeError(
    autonomy_msgs::msg::Error::NONE, ok ? constants::msg::kOk : constants::msg::kSigTimeout);
  task_manager_->endTask(
    ok ? autonomy_msgs::msg::TaskState::SUCCEEDED : autonomy_msgs::msg::TaskState::FAILED,
...
void CommandInterface::startTeleopIngress(const std::string & cmd_vel_teleop_topic)
{
  if (cmd_vel_teleop_topic.empty()) {
    return;
  }
  cmd_vel_teleop_sub_ = node_.create_subscription<geometry_msgs::msg::TwistStamped>(
    cmd_vel_teleop_topic, rclcpp::QoS(constants::defaults::kQueueDepth),
    [this](geometry_msgs::msg::TwistStamped::ConstSharedPtr msg) {
      if (!msg || !task_manager_->isTeleopActive()) {
        return;
      }
      task_manager_->updateTeleopCommand(*msg);
    });
  RCLCPP_INFO(
    node_.get_logger(), "[command] teleop ingress=%s", cmd_vel_teleop_topic.c_str());
}
  ...
  const std::string & target_id) const
{
  ...
  const auto & global_frame = task_manager_->coreOptions().global_frame;
  ...
  if (task_manager_->transformPoseToGlobalFrame(input)) {
    return conversions::toRos(input);
  }
  return pose;
}
