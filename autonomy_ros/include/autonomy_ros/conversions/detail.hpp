/*
 * Copyright 2024 The OpenRobotic Beginner Authors (duyongquan)
 * email: quandy2020@126.com
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef AUTONOMY_ROS__CONVERSIONS__DETAIL_HPP_
#define AUTONOMY_ROS__CONVERSIONS__DETAIL_HPP_

#include <google/protobuf/repeated_field.h>
#include <cstdint>
#include <vector>

#include <automsgs/msgs/builtin_interfaces/duration.pb.h>
#include <automsgs/msgs/builtin_interfaces/time.pb.h>
#include <automsgs/msgs/geometry_msgs/point.pb.h>
#include <automsgs/msgs/geometry_msgs/point32.pb.h>
#include <automsgs/msgs/geometry_msgs/polygon.pb.h>
#include <automsgs/msgs/geometry_msgs/pose.pb.h>
#include <automsgs/msgs/geometry_msgs/pose2d.pb.h>
#include <automsgs/msgs/geometry_msgs/quaternion.pb.h>
#include <automsgs/msgs/geometry_msgs/transform.pb.h>
#include <automsgs/msgs/geometry_msgs/twist.pb.h>
#include <automsgs/msgs/geometry_msgs/vector3.pb.h>
#include <automsgs/msgs/geometry_msgs/wrench.pb.h>
#include <automsgs/msgs/geometry_msgs/inertia.pb.h>
#include <automsgs/msgs/geometry_msgs/accel.pb.h>
#include <automsgs/msgs/std_msgs/color_rgba.pb.h>
#include <automsgs/msgs/std_msgs/header.pb.h>
#include "builtin_interfaces/msg/duration.hpp"
#include "builtin_interfaces/msg/time.hpp"
#include "geometry_msgs/msg/accel.hpp"
#include "geometry_msgs/msg/inertia.hpp"
#include "geometry_msgs/msg/point.hpp"
#include "geometry_msgs/msg/point32.hpp"
#include "geometry_msgs/msg/polygon.hpp"
#include "geometry_msgs/msg/pose.hpp"
#include "geometry_msgs/msg/pose2_d.hpp"
#include "geometry_msgs/msg/quaternion.hpp"
#include "geometry_msgs/msg/transform.hpp"
#include "geometry_msgs/msg/twist.hpp"
#include "geometry_msgs/msg/vector3.hpp"
#include "geometry_msgs/msg/wrench.hpp"
#include "rclcpp/time.hpp"
#include "std_msgs/msg/color_rgba.hpp"
#include "std_msgs/msg/header.hpp"

namespace autonomy_ros
{

void copyTime(
  const builtin_interfaces::msg::Time & from,
  automsgs::msgs::builtin_interfaces::Time & to);

void copyTime(
  const rclcpp::Time & from,
  automsgs::msgs::builtin_interfaces::Time & to);

builtin_interfaces::msg::Time toRosTime(
  const automsgs::msgs::builtin_interfaces::Time & from);

void copyHeader(
  const std_msgs::msg::Header & from,
  automsgs::msgs::std_msgs::Header & to);

void copyHeader(
  const automsgs::msgs::std_msgs::Header & from,
  std_msgs::msg::Header & to);

void copyQuaternion(
  const geometry_msgs::msg::Quaternion & from,
  automsgs::msgs::geometry_msgs::Quaternion & to);

void copyQuaternion(
  const automsgs::msgs::geometry_msgs::Quaternion & from,
  geometry_msgs::msg::Quaternion & to);

void copyPoint(
  const geometry_msgs::msg::Point & from,
  automsgs::msgs::geometry_msgs::Point & to);

void copyPoint(
  const automsgs::msgs::geometry_msgs::Point & from,
  geometry_msgs::msg::Point & to);

void copyPoint32(
  const geometry_msgs::msg::Point32 & from,
  automsgs::msgs::geometry_msgs::Point32 & to);

void copyPoint32(
  const automsgs::msgs::geometry_msgs::Point32 & from,
  geometry_msgs::msg::Point32 & to);

void copyVector3(
  const geometry_msgs::msg::Vector3 & from,
  automsgs::msgs::geometry_msgs::Vector3 & to);

void copyVector3(
  const automsgs::msgs::geometry_msgs::Vector3 & from,
  geometry_msgs::msg::Vector3 & to);

void copyPose2D(
  const geometry_msgs::msg::Pose2D & from,
  automsgs::msgs::geometry_msgs::Pose2D & to);

void copyPose2D(
  const automsgs::msgs::geometry_msgs::Pose2D & from,
  geometry_msgs::msg::Pose2D & to);

void copyPose(
  const geometry_msgs::msg::Pose & from,
  automsgs::msgs::geometry_msgs::Pose & to);

void copyPose(
  const automsgs::msgs::geometry_msgs::Pose & from,
  geometry_msgs::msg::Pose & to);

void copyTwist(
  const geometry_msgs::msg::Twist & from,
  automsgs::msgs::geometry_msgs::Twist & to);

void copyTwist(
  const automsgs::msgs::geometry_msgs::Twist & from,
  geometry_msgs::msg::Twist & to);

void copyAccel(
  const geometry_msgs::msg::Accel & from,
  automsgs::msgs::geometry_msgs::Accel & to);

void copyAccel(
  const automsgs::msgs::geometry_msgs::Accel & from,
  geometry_msgs::msg::Accel & to);

void copyTransform(
  const geometry_msgs::msg::Transform & from,
  automsgs::msgs::geometry_msgs::Transform & to);

void copyTransform(
  const automsgs::msgs::geometry_msgs::Transform & from,
  geometry_msgs::msg::Transform & to);

void copyWrench(
  const geometry_msgs::msg::Wrench & from,
  automsgs::msgs::geometry_msgs::Wrench & to);

void copyWrench(
  const automsgs::msgs::geometry_msgs::Wrench & from,
  geometry_msgs::msg::Wrench & to);

void copyInertia(
  const geometry_msgs::msg::Inertia & from,
  automsgs::msgs::geometry_msgs::Inertia & to);

void copyInertia(
  const automsgs::msgs::geometry_msgs::Inertia & from,
  geometry_msgs::msg::Inertia & to);

void copyPolygon(
  const geometry_msgs::msg::Polygon & from,
  automsgs::msgs::geometry_msgs::Polygon & to);

void copyPolygon(
  const automsgs::msgs::geometry_msgs::Polygon & from,
  geometry_msgs::msg::Polygon & to);

void copyDuration(
  const builtin_interfaces::msg::Duration & from,
  automsgs::msgs::builtin_interfaces::Duration & to);

void copyDuration(
  const automsgs::msgs::builtin_interfaces::Duration & from,
  builtin_interfaces::msg::Duration & to);

void copyColorRGBA(
  const std_msgs::msg::ColorRGBA & from,
  automsgs::msgs::std_msgs::ColorRGBA & to);

void copyColorRGBA(
  const automsgs::msgs::std_msgs::ColorRGBA & from,
  std_msgs::msg::ColorRGBA & to);

void copyCovariance6(
  const std::array<double, 36> & from,
  google::protobuf::RepeatedField<double> * to);

void copyCovariance6(
  const google::protobuf::RepeatedField<double> & from,
  std::array<double, 36> & to);

}  // namespace autonomy_ros

#endif  // AUTONOMY_ROS__CONVERSIONS__DETAIL_HPP_
