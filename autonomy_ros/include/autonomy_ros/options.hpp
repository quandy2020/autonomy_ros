 ...
 *
 * Loaded from autonomy.* ROS parameters (see autonomy_params.yaml).
 */
struct AutonomyRosOptions
{
  ...
  /** @brief Teleop ingress topic (operator → core); empty = disabled. */
  std::string cmd_vel_teleop_topic{constants::topics::kCmdVelTeleop};
