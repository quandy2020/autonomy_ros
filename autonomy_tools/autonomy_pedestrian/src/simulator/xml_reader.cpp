...
void XMLReader::Read()
{
    ...
    std::string map_file;
    if (!CONFIG.scenario_file_.empty() &&
        (CONFIG.scenario_file_.front() == '/' || CONFIG.scenario_file_.front() == '\\'))
    {
        map_file = CONFIG.scenario_file_;
    }
    else
    {
        map_file = getPackagePath(CONFIG.scenario_package_name_) + "/scenarios/" + CONFIG.scenario_file_;
    }
...
void XMLReader::Read(const std::string &file_name)
{
    ...
    std::string map_file;
    if (file_name.find("//autonomy_pedestrian//") != std::string::npos)
        map_file = getPackagePath("autonomy_pedestrian") + "/" + file_name;
    else
        map_file = file_name;
