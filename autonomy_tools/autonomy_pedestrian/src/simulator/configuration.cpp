...
void Config::Init() // const std::string& node_handle_name
{
  ...
  ros::NodeHandle node;
#else
  auto node = GET_STATIC_NODE_POINTER();
#endif
  retrieveParameter(node, "pedestrian_simulator/node/debug_output", debug_output_, true);
  retrieveParameter(node, "pedestrian_simulator/node/scenario_package_name", scenario_package_name_, std::string("autonomy_pedestrian"));
  ...
  retrieveParameter(node, "pedestrian_simulator/pedestrians/respawn", respawn_peds_, false);
