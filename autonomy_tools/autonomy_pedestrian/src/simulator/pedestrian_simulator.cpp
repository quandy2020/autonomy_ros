...
std::vector<Prediction> PedestrianSimulator::GetPedestrians()
{
    ...
    for (auto &ped : pedestrians_)
    {
        ...
        Eigen::Vector2d velocity = ped->GetSpeed();
...
std::vector<Prediction> PedestrianSimulator::GetPredictions()
{
    ...
    /*
        pedsim_msgs::msg::ObstacleArray prediction_array;
        ...
        std::vector<pedsim_msgs::msg::Gaussian> gaussian_msgs;
        ...
        for (auto &ped : pedestrians_)
        {
            ...
            pedsim_msgs::msg::ObstacleGMM gmm_msg;
            gmm_msg.id = id;
...
void PedestrianSimulator::VisualizeWalls()
{
    if (!xml_reader_ || xml_reader_->static_obstacles_.empty())
        return;

    auto &wall_visual = VISUALS.getPublisher("walls");
    auto &wall_line = wall_visual.getNewLine();
    wall_line.setScale(0.08, 0.08);
    wall_line.setColor(0.2, 0.2, 0.2, 1.0);

    for (const auto &obstacle : xml_reader_->static_obstacles_)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            const Eigen::Vector2d &p1 = corners[i];
            const Eigen::Vector2d &p2 = corners[(i + 1) % 4];
            wall_line.addLine(p1, p2, 0.05);
        }
    }
...
void PedestrianSimulator::VisualizePredictions()
{
    if (CONFIG.horizon_N_ <= 0)
        return;

    const auto predictions = GetPredictions();
    if (predictions.empty())
        return;

    auto &prediction_visual = VISUALS.getPublisher("prediction_paths");
    for (const auto &prediction : predictions)
    {
        if (prediction.pos.size() < 2)
            continue;

        auto &path_line = prediction_visual.getNewLine();
        path_line.setScale(0.05, 0.05);
        path_line.setColorInt(prediction.id, 1.0, RosTools::Colormap::VIRIDIS);

        for (size_t i = 1; i < prediction.pos.size(); ++i)
        {
            path_line.addLine(prediction.pos[i - 1], prediction.pos[i], 0.1);
        }
    }

    prediction_visual.publish();
}