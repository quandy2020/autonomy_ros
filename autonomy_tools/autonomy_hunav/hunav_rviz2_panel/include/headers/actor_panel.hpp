...
namespace hunav_rviz2_panel
{
  ...
  protected Q_SLOTS:
    ...
    /**
     * @brief Rebuild and publish goal sphere/label markers from loaded_global_goals_
     */
    void rebuildAndPublishGoalMarkers();
    ...
    /**
     * @brief Subscribe to an OccupancyGrid topic (Pure RViz mode)
     */
    void onSubscribeMapTopic();

    /**
     * @brief Handle map messages from topic subscription
     */
    void onMapTopicReceived(const nav_msgs::msg::OccupancyGrid::SharedPtr msg);

    /**
     * @brief Update map-loaded UI state
     */
    ...
    /**
     * @brief Activate an RViz tool by plugin class id (loads if missing)
     */
    void activateRvizTool(const QString & class_id);
     ...
     */
    bool generatePedestrianXmlFromYaml(const QString & yaml_path) const;

    /**
     * @brief Launch autonomy_pedestrian nodes for Pure RViz simulation
     */
    void startPureRvizPedestrianSim();

    /**
     * @brief Stop a running Pure RViz pedestrian simulation
     */
    void stopPureRvizPedestrianSim();

    /**
     * @brief Fill Agent appearance combobox for the active simulator
  ...
  public:
    ...
    QPushButton *map_subscribe_btn_{nullptr};       ///< Map topic subscribe button (Pure RViz)
    QLineEdit *map_topic_edit_{nullptr};            ///< Map topic input (Pure RViz)
    QPushButton *run_pedestrian_sim_btn_{nullptr};  ///< Start/stop autonomy_pedestrian (Pure RViz)
    QWidget *map_topic_row_{nullptr};               ///< Row widget for topic subscription UI
    ...
    QString map_topic_name_;              ///< Subscribed map topic name
    QString last_saved_yaml_path_;        ///< Last saved scenario YAML (Pure RViz sim)
    ...
    bool map_loaded_from_topic_{false};     ///< Map loaded via topic subscription
    bool map_topic_ui_applied_{false};      ///< Map topic UI already updated
    QProcess * pedestrian_sim_process_{nullptr}; ///< Running Pure RViz pedestrian launch
    ...
    rclcpp::Subscription<nav_msgs::msg::OccupancyGrid>::SharedPtr map_sub_;                    ///< Map subscription

    // Node handles
    rclcpp::Node::SharedPtr ros_node_;                   ///< RViz raw ROS node
