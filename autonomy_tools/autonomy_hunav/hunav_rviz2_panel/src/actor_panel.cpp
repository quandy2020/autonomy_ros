...
namespace hunav_rviz2_panel
{
      ...
      : rviz_common::Panel(parent), rclcpp::Node("hunav_rviz2_panel")
  {
    ...
    QLabel *map_topic_label = new QLabel("Map topic:");
    map_topic_label->setStyleSheet("font-weight: bold; color: #2c3e50;");
    map_topic_edit_ = new QLineEdit("/map", map_topic_row_);
    map_topic_edit_->setPlaceholderText("/map");
    map_topic_edit_->setStyleSheet(
        "QLineEdit {"
        "  padding: 4px 8px;"
        "  border: 2px solid #bdc3c7;"
        "  border-radius: 6px;"
        "  font-size: 14px;"
        "}");
    map_subscribe_btn_ = new QPushButton("Subscribe", map_topic_row_);
    map_subscribe_btn_->setEnabled(false);
    map_subscribe_btn_->setStyleSheet(
        "QPushButton {"
        "  padding: 4px 8px;"
        "  border: 2px solid #e74c3c;"
        "  border-radius: 6px;"
    ...
    connect(run_pedestrian_sim_btn_, &QPushButton::clicked, this, [this](bool checked)
            {
      run_pedestrian_sim_btn_->setDown(checked);
      if (checked)
        startPureRvizPedestrianSim();
      else
    ...
    connect(edit_goals_button_, &QPushButton::clicked, this, [this](bool checked)
        ...
        if (goal_picking_mode_)
        {
          activateRvizTool(QStringLiteral("rviz_default_plugins/PublishPoint"));
        }
        else
        {
          activateRvizTool(QStringLiteral("rviz_default_plugins/Interact"));
  ...
  void ActorPanel::onEditAllInGroot()
  {
    ...
    if (!loaded_agent_nodes_.empty())
    {
      ...
      for (auto const &it : loaded_global_goals_)
      {
        ...
        sphere.header.frame_id = mapFrameId().toStdString();
  ...
  void ActorPanel::addAgent()
  {
    ...
    QGroupBox *sim_group = new QGroupBox("Simulation Options");
  ...
  {
    if (!initial_pose_publisher)
    {
      QMessageBox::warning(
          this,
          tr("Initial Pose"),
          tr("ROS publisher is not ready yet. Close and re-add the HuNav panel in RViz."));
      if (initial_pose_button)
      {
        initial_pose_button->setChecked(false);
      }
      return;
    }
    ...
    if (initial_pose_button)
    {
      initial_pose_button->setChecked(true);
    }
    ...
    activateRvizTool(QStringLiteral("hunav_rviz2_panel/HunavGoals"));
  }
  ...
  void ActorPanel::onInitialPose(double x, double y, double theta, QString frame)
  {
    if (!initial_pose_publisher)
    {
      RCLCPP_ERROR(get_logger(), "onInitialPose(): publisher not initialized");
      return;
    }
    ...
    {
      idx = (agent_count > 0) ? (agent_count - 1) : 0;
      current_edit_idx_ = idx;
    }

    if (idx < 0)
    {
      RCLCPP_ERROR(get_logger(), "onInitialPose(): invalid agent index %d", idx);
      return;
    }

    if (loaded_initial_marker_ids_.size() <= static_cast<size_t>(idx))
    {
      loaded_initial_marker_ids_.resize(static_cast<size_t>(idx) + 1, -1);
    }

    if (agent_colors_.size() <= static_cast<size_t>(idx))
    {
      initAgentColors((num_agents > idx + 1) ? num_agents : (idx + 1));
    }
    ...
    if (panel_mode_ == EDIT_MODE && !adding_new_agent_)
    {
      ...
      m.header.frame_id = mapFrameId().toStdString();
    ...
    visualization_msgs::msg::Marker label =
        createAgentLabel(x, y, id, mapFrameId().toStdString());
    label.header.stamp = mesh.header.stamp;
    ...
    RCLCPP_INFO(
        get_logger(),
        "Published agent %d initial pose at (%.2f, %.2f, yaw=%.2f) on /hunav_agent",
        idx + 1, x, y, theta);
  ...
  QString ActorPanel::mapFrameId()
  {
    return QStringLiteral("map");
  }

  void ActorPanel::activateRvizTool(const QString & class_id)
  {
    auto * tm = getDisplayContext() ? getDisplayContext()->getToolManager() : nullptr;
    if (!tm)
    {
      return;
    }
    ...
    {
      if (QString(tm->getTool(i)->getClassId()) == class_id)
      {
  ...
  void ActorPanel::enableLoadedConfigurationUi()
  {
    create_button_->setChecked(false);
    create_button_->setDown(false);
    open_button_->setChecked(true);
  ...
  bool ActorPanel::generatePedestrianXmlFromYaml(const QString & yaml_path) const
  {
    QString converter;
    try
    {
      const std::string prefix =
          ament_index_cpp::get_package_prefix("hunav_rviz2_panel");
      converter = QString::fromStdString(
          prefix + "/lib/hunav_rviz2_panel/hunav_yaml_to_pedestrian_xml.py");
    }
    ...
    const QString xml_path = yaml_path.endsWith(".yaml")
                                 ? yaml_path.left(yaml_path.size() - 5) + ".xml"
                                 : yaml_path + ".xml";

    QProcess proc;
    proc.setProgram("python3");
    proc.setArguments({converter, yaml_path, xml_path});
    proc.start();
    ...
    return proc.exitCode() == 0 && QFile::exists(xml_path);
  }
  ...
  void ActorPanel::startPureRvizPedestrianSim()
  {
    ...
    try
    {
      const std::string panel_prefix =
          ament_index_cpp::get_package_prefix("hunav_rviz2_panel");
      const auto install_pos = panel_prefix.find("/install/");
      if (install_pos != std::string::npos)
      {
        ws_root = QString::fromStdString(panel_prefix.substr(0, install_pos));
      }
    }
    catch (const std::exception &)
    {
    ...
    pedestrian_sim_process_ = new QProcess(this);
    pedestrian_sim_process_->setProcessChannelMode(QProcess::MergedChannels);
    connect(pedestrian_sim_process_, &QProcess::readyReadStandardOutput, this, [this]()
            {
              if (!pedestrian_sim_process_)
                return;
              const QByteArray output = pedestrian_sim_process_->readAllStandardOutput();
              if (!output.isEmpty())
                RCLCPP_INFO(get_logger(), "[pedestrian_sim] %s", output.constData());
            });
    pedestrian_sim_process_->start("/bin/bash", {"-lc", shell_cmd});

    if (!pedestrian_sim_process_->waitForStarted(5000))
    {
      QMessageBox::critical(
          this,
          tr("Pure RViz Simulation"),
  ...
  void ActorPanel::stopPureRvizPedestrianSim()
  {
    if (!pedestrian_sim_process_)
    {
      return;
    ...
    if (pedestrian_sim_process_->state() != QProcess::NotRunning)
    {
      pedestrian_sim_process_->terminate();
      if (!pedestrian_sim_process_->waitForFinished(3000))
      {
        pedestrian_sim_process_->kill();
        pedestrian_sim_process_->waitForFinished(1000);
      }
    }

    pedestrian_sim_process_->deleteLater();
    pedestrian_sim_process_ = nullptr;
  ...
  void ActorPanel::onSubscribeMapTopic()
  {
    if (!ros_node_)
    {
      QMessageBox::warning(this, "Map Topic", "ROS node not ready. Re-open the panel in RViz.");
      return;
    }

    QString topic = map_topic_edit_->text().trimmed();
    if (topic.isEmpty())
    {
      QMessageBox::warning(this, "Map Topic", "Enter an OccupancyGrid topic name (e.g. /map).");
      return;
    }
    if (!topic.startsWith('/'))
    {
    ...
    map_sub_ = ros_node_->create_subscription<nav_msgs::msg::OccupancyGrid>(
        topic.toStdString(),
        qos,
        std::bind(&ActorPanel::onMapTopicReceived, this, std::placeholders::_1));

    current_map_label_->setText(QString("Waiting for %1…").arg(topic));
    current_map_label_->setStyleSheet(
        "QLabel {"
        "  padding: 4px 8px;"
  ...
  void ActorPanel::onMapTopicReceived(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
  {
    ...
    map_name_ = QString::fromStdString(msg->header.frame_id);
    if (map_name_.isEmpty())
    {
      map_name_ = "map";
    }
    if (map_name_.startsWith('/'))
    {
      map_name_ = map_name_.mid(1);
    }
    ...
    map_loaded_from_topic_ = true;

    if (!map_topic_ui_applied_)
    {
      map_topic_ui_applied_ = true;
      QTimer::singleShot(0, this, [this]()
                         { applyMapLoadedUi(map_topic_name_); });
    }
  }

  void ActorPanel::applyMapLoadedUi(const QString & display_name)
  {
    panel_mode_ = CREATE_MODE;
    actors->setEnabled(true);
    n_agents_label_->setEnabled(true);

    current_map_label_->setText(display_name);
    current_map_label_->setStyleSheet(
        "QLabel {"
        "  padding: 4px 6px;"
  ...
  void ActorPanel::rebuildAndPublishGoalMarkers()
  {
    ...
    for (auto const &it : loaded_global_goals_)
    {
      ...
      label.pose.position.y = pt.y + 0.5;
      label.pose.position.z = 0.6;
  ...
  void ActorPanel::onEnterGoalPickingMode()
  {
    ...
    bool anyGoals = !loaded_global_goals_.empty();
  ...
  void ActorPanel::onResetLoadedGoals()
  {
    ...
    delete_all.header.frame_id = mapFrameId().toStdString();
  ...
  void ActorPanel::parseYaml()
  {
    ...
    last_saved_yaml_path_ = chosenFile;
  ...
  void ActorPanel::saveAndGenerateAll()
  {
    ...
    else if (!map_name_.isEmpty())
      defaultName_ = map_name_ + "_agents_";
    else
      defaultName_ = "scenario_agents_";
  ...
  void ActorPanel::populateSkinComboBox(const QString & simulator_name, int select_index)
  {
    ...
    {
      skin_combobox->setCurrentIndex(select_index);
    }
    else if (skin_combobox->count() > 0)
    {
      skin_combobox->setCurrentIndex(0);
    }

    skin_combobox->setEnabled(skin_combobox->count() > 0);
    if (skin_label_)
    {
      skin_label_->setVisible(true);
    }
                                                           ...
                                                           std::string marker_shape, std::string create_or_parser)
  {
    ...
    marker.header.frame_id = mapFrameId().toStdString();
                                                                ...
                                                                double point2_y, double ids)
  {
    ...
    arrow_marker.header.frame_id = mapFrameId().toStdString();
