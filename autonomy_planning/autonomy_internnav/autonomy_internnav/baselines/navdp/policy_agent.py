...
from autonomy_internnav.baselines.navdp.colormap import critic_value_to_bgr_u8
...
class NavDP_Agent:
    ...
    def reset(self,batch_size,threshold,sample_num=16):
                ...
                for i in range(camera_x.shape[0]-1):
                    try:
                        if camera_x[i] > 0 and camera_z[i] > 0 and camera_x[i+1] > 0 and camera_z[i+1] > 0:
                            trajectory_mask = cv2.line(trajectory_mask,(int(camera_x[i]),int(camera_z[i])),(int(camera_x[i+1]),int(camera_z[i+1])),color,5)
                    except:
                        pass
    ...
    def step_nogoal(self,images,depths):
        ...
        all_trajectory, all_values, good_trajectory, bad_trajectory = self.navi_former.predict_nogoal_action(
            input_image, input_depth, sample_num=self.sample_num)
    ...
    def step_pointgoal(self,goals,images,depths):
        ...
        # cv2.imwrite("input_image.jpg",np.concatenate(self.memory_queue[0],axis=0)*255)
        all_trajectory, all_values, good_trajectory, bad_trajectory = self.navi_former.predict_pointgoal_action(
            input_goals, input_image, input_depth, sample_num=self.sample_num)
        if all_values.max() < self.stop_threshold:
            good_trajectory[:,:,:,0] = good_trajectory[:,:,:,0] * 0.0
            good_trajectory[:,:,:,1] = np.sign(good_trajectory[:,:,:,1].mean())
    ...
    def step_imagegoal(self,goals,images,depths):
        ...
        all_trajectory, all_values, good_trajectory, bad_trajectory = self.navi_former.predict_imagegoal_action(
            input_goals, input_image, input_depth, sample_num=self.sample_num)
        if all_values.max() < self.stop_threshold:
            good_trajectory[:,:,:,0] = good_trajectory[:,:,:,0] * 0.0
            good_trajectory[:,:,:,1] = np.sign(good_trajectory[:,:,:,1].mean())
    ...
    def step_pixelgoal(self,goals,images,depths):
        ...
        all_trajectory, all_values, good_trajectory, bad_trajectory = self.navi_former.predict_pixelgoal_action(
            input_goals, input_image, input_depth, sample_num=self.sample_num)
    ...
    def step_point_image_goal(self,pointgoal,imagegoal,images,depths):
        ...
        all_trajectory, all_values, good_trajectory, bad_trajectory = self.navi_former.predict_ip_action(
            input_pointgoal, input_imagegoal, input_image, input_depth, sample_num=self.sample_num)
