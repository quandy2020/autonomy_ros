"""S10 precision pose alignment demo — scripted upper controller + s10_jit locomotion."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='S10 precision pose alignment demo.')
...
def main() -> int:
    ...
    # Always show target pose / heading markers for precision demo (WebRTC or GUI).
    viz = config.setdefault('viz', {})
    viz['enabled'] = True
    viz.setdefault('auto_enable_on_livestream', True)
    viz.setdefault(
        'modules',
        [
            'target_footprint',
    ...
    viz.setdefault('spot_length', 0.55)
    viz.setdefault('spot_width', 0.35)
    viz.setdefault('robot_length', 0.55)
    viz.setdefault('robot_width', 0.35)
    viz.setdefault('arrow_length', 0.50)
    viz.setdefault('marker_z_offset', 0.08)

    task_cfg = config.setdefault('task', {})
    target = list(task_cfg.get('target_pose', [1.0, 0.0, 0.0]))
    ...
    task_cfg['kind'] = 'precision_pose'
    task_cfg['mode'] = 'precision_pose'

    control_cfg = config.get('control', {})
    max_vx = float(control_cfg.get('max_vx', 0.5))
    max_vy = float(control_cfg.get('max_vy', 0.3))
    max_w = float(control_cfg.get('max_w', 1.0))
    pos_tol = float(task_cfg.get('position_tolerance_m', task_cfg.get('goal_tolerance_m', 0.08)))
    yaw_tol = float(task_cfg.get('yaw_tolerance_rad', 0.12))
    speed_tol = float(task_cfg.get('require_stop_speed_mps', 0.10))

    print(
        f'[demo] S10 precision alignment target: x={target[0]:.3f} y={target[1]:.3f} '
        f'yaw={math.degrees(target[2]):.1f} deg  loco={config.get("locomotion", {}).get("backend")}',
        flush=True,
    )
    print(
    ...
    try:
        ...
        for step in range(args.max_steps):
            ...
            if result.terminated.any() or aligned:
                success = aligned or bool(result.terminated.any())
                print(
